		...
		foreach($result as $data)
		{
			?>
				<tr>
					<td>{{ $no }}</td>
					<td>{{ $data["name"] }}</td>
					<td><?php echo isset($data["event"]["is_finished"]) && $data["event"]["is_finished"] != 0 ? "<span style='color: red;'>".$data["event"]["full_name"]."</span>" : $data["event"]["full_name"] ?></td>
					<td>
						<a class="__action_btn btn-edit-event"  href="{{ URL::Route(GROUP_INTERNAL . '.form.team', $data['id']) }}" title="Edit">
